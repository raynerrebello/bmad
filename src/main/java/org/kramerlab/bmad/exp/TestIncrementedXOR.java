...
import org.kramerlab.bmad.algorithms.ErrorReconstruction;
import org.kramerlab.bmad.algorithms.Heuristic;
...
public class TestIncrementedXOR {
    public static void main(String[] args) throws Exception {
        ...
        double coverage = 0;
        ErrorReconstruction hillClimber;
        Heuristic H;
        H = new HillClimber();
        ...
        for (int j = Integer.parseInt(args[0]); j < Integer.parseInt(args[1]); j++) {
            ...
            for (int i = 0; i < listOfFiles.length; i++) {
                if (listOfFiles[i].isFile()) {
                    ...
                    for (int k : kValues) {
                        ...
                        Tuple<BooleanMatrix, BooleanMatrix> ans = hillClimber.incrementedXBMaD(T, k,Math.min(k,Integer.parseInt(args[2])));

                        long endTime = System.nanoTime();

                        double duration = (endTime - startTime) / 1e9;
                        BooleanMatrix R = ans._1.booleanProduct(ans._2,true);
            ...
            System.out.println(String.format("%d of %d restarts done...",(j+1)-Integer.parseInt(args[0]),Integer.parseInt(args[1])-Integer.parseInt(args[0]) ));
